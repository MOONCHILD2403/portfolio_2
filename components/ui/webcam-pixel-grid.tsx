...
export const WebcamPixelGrid: React.FC<WebcamPixelGridProps> = ({
  ...
  const currentBgColor = backgroundColor === "#0a0a0a"
    ? (theme === "light" ? "#f3f4f6" : "#0d0e10")
    : backgroundColor;

  const currentBorderColor = borderColor === "#ffffff"
    ? (theme === "light" ? "#0d0e10" : "#ffffff")
    : borderColor;
  ...
  const borderRGB = React.useMemo(() => {
    const hex = currentBorderColor.replace("#", "");
    return {
      r: parseInt(hex.slice(0, 2), 16),
      g: parseInt(hex.slice(2, 4), 16),
      b: parseInt(hex.slice(4, 6), 16),
  ...
  const render = useCallback(() => {
    ...
    // Clear canvas
    dispCtx.fillStyle = currentBgColor;
